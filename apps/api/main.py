...
from core.storage.sqlite import TaskStore

app = FastAPI(title="K-OS Cloud Pro API", version="0.3.0")
app.add_middleware(CORSMiddleware,allow_origins=["*"],allow_credentials=False,allow_methods=["*"],allow_headers=["*"])
DASHBOARD_FILE = Path(__file__).resolve().parents[1] / "dashboard" / "index.html"
STORE = TaskStore()
TASKS: list[dict] = STORE.list()
...
class TaskStatusIn(BaseModel):
    status: str = Field(pattern="^(queued|running|blocked|completed|failed|cancelled)$")

@app.get("/",include_in_schema=False)
def dashboard()->FileResponse:
    if not DASHBOARD_FILE.exists(): raise HTTPException(status_code=404,detail="Dashboard não encontrado")
    return FileResponse(DASHBOARD_FILE)

@app.get("/health")
def health()->dict:
    return {"status":"ok","service":"kos-api","version":app.version,"time":datetime.now(timezone.utc).isoformat(),"storage":STORE.health()}

@app.get("/api/agents")
def agents()->list[dict]: return AGENTS

@app.get("/api/connectors")
def connectors()->list[dict]: return list_connectors()

@app.get("/api/connectors/{connector_id}")
def connector(connector_id:str)->dict:
    definition=get_connector(connector_id)
    if definition is None: raise HTTPException(status_code=404,detail="Conector não encontrado")
    return definition.public_dict()

@app.get("/api/approvals")
def approvals()->list[dict]: return list_approvals()
...
def create_command(command:CommandIn)->dict:
    task={"id":str(uuid4()),"command":command.text.strip(),"project":command.project.strip(),"status":"queued","assigned_agent":"ORQ-001","created_at":datetime.now(timezone.utc).isoformat(),"payload":command.payload}
    STORE.insert(task)
    TASKS.insert(0,task)
    return task

@app.patch("/api/tasks/{task_id}")
def update_task(task_id:str,request:TaskStatusIn)->dict:
    task=STORE.update_status(task_id,request.status)
    if task is None: raise HTTPException(status_code=404,detail="Tarefa não encontrada")
    return task
